...
public class LockstepTest
{
    ...
    public void Relayed()
    {
        ...
        for (int i = 0; i < 200; i++)
        {
            ...
            client1Obj.SendUpdate(client1Obj.client1Val + 1, client1.CurTick);
            client1.Log($"sent {client1Obj.client1Val + 1} at {client1.CurTick}");

            if (client2.TryGetObject(client1Obj.Id, out LockstepTestObject client2Obj))
            {
                client2Obj.SendUpdate(client2Obj.client2Val + 1, client2.CurTick);
                client2.Log($"sent {client2Obj.client2Val + 1} at {client2.CurTick}");
            }
